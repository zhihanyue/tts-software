...
//
// $Revision:   1.61.1.0.1.13  $
// $Date:   04 Jun 2002 13:44:16  $
//
...
HRESULT DebugHlpr_HRCHECK(HRESULT hr, T* expr, T* file, int line)
{
  ...
  {
  	//暂时修改为：
  	MessageBox(MainForm->Handle,"对不起，标签格式出现错误。\n\n检查并修改错误或关闭标签解析器可解决这个问题。","错误提示：",MB_ICONERROR);
...
class TVariantT : public TBaseVariantT<T>
{
 public:
  ...
  TVariantT(const TVariantT& src)
  {
    /* always copies the other variant. */
    ::VariantInit(this);
    ::VariantCopy(this, const_cast<TVariant*>(&src));
  }
  ...
  TVariantT& operator =(TVariantT rhs)
  {
    ::VariantClear(this);
    ::VariantCopy(this, &rhs);
    return *this;
